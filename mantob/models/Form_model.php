...
class Form_model extends CI_Model {
	...
	public function add($data) {
		...
		if ($this->link->insert($this->prefix, $data)) {
			...
			$file = FCPATH.'mantob/controllers/admin/'.$name.'.php';
			if (!file_put_contents($file, '<?php'.PHP_EOL.PHP_EOL
			.'require FCPATH.\'mantob/core/D_Form.php\';'.PHP_EOL.PHP_EOL
	...
	public function del($id, $sid = SITE_ID) {
		...
		$name = 'Form_'.$sid.'_'.$id;
		@unlink(FCPATH.'mantob/controllers/'.$name.'.php');
		@unlink(FCPATH.'mantob/controllers/admin/'.$name.'.php');
		@unlink(FCPATH.'mantob/templates/admin/form_'.$id.'.html');
