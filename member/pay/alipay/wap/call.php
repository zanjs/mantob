...
$pay = $this->get_cache('member', 'setting', 'pay', 'alipay');
...
$alipay_config = array(
    ...
    'sign_type' => 'MD5',
    'transport' => 'http',
    'return_url' => SITE_URL.'member/pay/alipay/return_url.php',
    'notify_url' => SITE_URL.'member/pay/alipay/notify_url.php',
    'seller_email' => $pay['username'],
    'input_charset' => 'utf-8',
);

require APPPATH.'pay/alipay/wap/alipay_notify.class.php';
...
if (DR_PAY_FILE == 'return') {
    ...
    if($verify_result) {
        ...
        $this->pay_msg(
            "支付成功",
            $module ? MEMBER_URL.'index.php?s='.$module.'&c=order&m=index' : MEMBER_URL.'index.php?c=pay',
            1
        );
    } else {
        $this->pay_msg('验证失败');
